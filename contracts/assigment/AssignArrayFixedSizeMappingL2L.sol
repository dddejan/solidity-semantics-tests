pragma solidity >=0.5.0;

contract AssignArrayFixedSizeMappingL2L {

    mapping(address=>int)[2] s1;
    mapping(address=>int)[2] s2;

    function locToLoc(int a, int b, int c) public {
        require(a != b && b != c && a != c);
        s1[1][msg.sender] = a;
        s2[1][msg.sender] = b;
        mapping(address=>int)[2] storage sl1 = s1;
        mapping(address=>int)[2] storage sl2 = s2;
        assert(sl1[1][msg.sender] == a);
        assert(sl2[1][msg.sender] == b);

        sl1 = sl2; // Checking this assignemnt
        sl1[1][msg.sender] = c;
        assert(sl1[1][msg.sender] == c);
        assert(sl2[1][msg.sender] == c);
        assert(s1[1][msg.sender] == a);
        assert(s2[1][msg.sender] == c);
    }

    // Boilerplate to run with truffle
    function truffleMain() external {
        locToLoc(0, 1, 2);
    }
}
